    ...
    Dispatch.prototype = dispatch.prototype = {
      ...
      on: function(typename, callback) {
        ...
        if (arguments.length < 2) {
          while (++i < n2) if ((t4 = (typename = T2[i]).type) && (t4 = get(_[t4], typename.name))) return t4;
          return;
        }
        if (callback != null && typeof callback !== "function") throw new Error("invalid callback: " + callback);
        while (++i < n2) {
          if (t4 = (typename = T2[i]).type) _[t4] = set(_[t4], typename.name, callback);
          else if (callback == null) for (t4 in _) _[t4] = set(_[t4], typename.name, null);
        }
        return this;
      },
      copy: function() {
        var copy3 = {}, _ = this._;
        for (var t4 in _) copy3[t4] = _[t4].slice();
        return new Dispatch(copy3);
      },
      call: function(type, that) {
        if ((n2 = arguments.length - 2) > 0) for (var args = new Array(n2), i = 0, n2, t4; i < n2; ++i) args[i] = arguments[i + 2];
        if (!this._.hasOwnProperty(type)) throw new Error("unknown type: " + type);
        for (t4 = this._[type], i = 0, n2 = t4.length; i < n2; ++i) t4[i].value.apply(that, args);
      },
      apply: function(type, that, args) {
        if (!this._.hasOwnProperty(type)) throw new Error("unknown type: " + type);
        for (var t4 = this._[type], i = 0, n2 = t4.length; i < n2; ++i) t4[i].value.apply(that, args);
      }
    };
  ...
  "node_modules/d3-cloud/index.js"(exports, module) {
        ...
        function step() {
          ...
          while (Date.now() - start3 < timeInterval && ++i < n2 && timer2) {
            ...
            if (d2.hasText && place(board, d2, bounds)) {
              ...
              if (bounds) cloudBounds(bounds, d2);
              else bounds = [{ x: d2.x + d2.x0, y: d2.y + d2.y0 }, { x: d2.x + d2.x1, y: d2.y + d2.y1 }];
              d2.x -= size[0] >> 1;
              d2.y -= size[1] >> 1;
            }
          }
    ...
    function cloudSprite(contextAndRatio, d2, data, di) {
      ...
      while (++di < n2) {
        ...
        if (d2.rotate) c3.rotate(d2.rotate * RADIANS);
        c3.fillText(d2.text, anchor, 0);
        if (d2.padding) c3.lineWidth = 2 * d2.padding, c3.strokeText(d2.text, anchor, 0);
        c3.restore();
        d2.width = w2;
        d2.height = h3;
        d2.xoff = x;
    ...
    function cloudCollide(tag, board, sw) {
      ...
      for (var j2 = 0; j2 < h2; j2++) {
        last = 0;
        for (var i = 0; i <= w; i++) {
          if ((last << msx | (i < w ? (last = sprite[j2 * w + i]) >>> sx : 0)) & board[x + i]) return true;
        }
        x += sw;
      }
      return false;
    ...
    function cloudBounds(bounds, d2) {
      ...
      if (d2.x + d2.x0 < b02.x) b02.x = d2.x + d2.x0;
      if (d2.y + d2.y0 < b02.y) b02.y = d2.y + d2.y0;
      if (d2.x + d2.x1 > b12.x) b12.x = d2.x + d2.x1;
      if (d2.y + d2.y1 > b12.y) b12.y = d2.y + d2.y1;
    }
  ...
  "node_modules/seedrandom/lib/alea.js"(exports, module) {
    (function(global2, module2, define2) {
      ...
      function impl(seed, opts) {
        ...
        prng.quick = prng;
        if (state) {
          if (typeof state == "object") copy3(state, xg);
          prng.state = function() {
            return copy3(xg, {});
          };
        }
...
function on_default(typename, value, capture) {
  ...
  for (i = 0; i < n2; ++i) this.each(on(typenames[i], value, capture));
  return this;
}
...
function hslConvert(o2) {
  ...
  if (s) {
    if (r2 === max3) h2 = (g - b) / s + (g < b) * 6;
    else if (g === max3) h2 = (b - r2) / s + 2;
    else h2 = (r2 - g) / s + 4;
...
function string_default(a, b) {
  ...
  return s.length < 2 ? q2[0] ? one(q2[0].x) : zero(b) : (b = q2.length, function(t4) {
    for (var i2 = 0, o2; i2 < b; ++i2) s[(o2 = q2[i2]).i] = o2.x(t4);
    return s.join("");
  });
}
...
function decompose_default(a, b, c3, d2, e3, f) {
  ...
  if (skewX = a * c3 + b * d2) c3 -= a * skewX, d2 -= b * skewX;
  if (scaleY = Math.sqrt(c3 * c3 + d2 * d2)) c3 /= scaleY, d2 /= scaleY, skewX /= scaleY;
  if (a * d2 < b * c3) a = -a, b = -b, skewX = -skewX, scaleX = -scaleX;
  return {
    translateX: e3,
    translateY: f,
...
function interpolateTransform(parse, pxComma, pxParen, degParen) {
  ...
  return function(a, b) {
    ...
    return function(t4) {
      ...
      while (++i < n2) s[(o2 = q2[i]).i] = o2.x(t4);
      return s.join("");
    };
  };
}
...
function styleTween(name, value, priority) {
  ...
  function tween() {
    var i = value.apply(this, arguments);
    if (i !== i0) t4 = (i0 = i) && styleInterpolate(name, i, priority);
    return t4;
  }
  tween._value = value;
  return tween;
...
function ticks_default(start3, stop, count2) {
  ...
  if (reverse2 = stop < start3) n2 = start3, start3 = stop, stop = n2;
...
function rgbConvert2(o2) {
  if (!(o2 instanceof Color2)) o2 = color2(o2);
...
function labConvert2(o2) {
  if (o2 instanceof Lab2) return new Lab2(o2.l, o2.a, o2.b, o2.opacity);
  if (o2 instanceof Hcl2) return hcl2lab2(o2);
...
function hclConvert2(o2) {
  if (o2 instanceof Hcl2) return new Hcl2(o2.h, o2.c, o2.l, o2.opacity);
  if (!(o2 instanceof Lab2)) o2 = labConvert2(o2);
  if (o2.a === 0 && o2.b === 0) return new Hcl2(NaN, 0 < o2.l && o2.l < 100 ? 0 : NaN, o2.l, o2.opacity);
...
function hcl2lab2(o2) {
  if (isNaN(o2.h)) return new Lab2(o2.l, 0, 0, o2.opacity);
  var h2 = o2.h * radians;
...
function cubehelixConvert2(o2) {
  if (o2 instanceof Cubehelix2) return new Cubehelix2(o2.h, o2.s, o2.l, o2.opacity);
  if (!(o2 instanceof Rgb2)) o2 = rgbConvert2(o2);
...
function numberArray_default2(a, b) {
  ...
  return function(t4) {
    for (i = 0; i < n2; ++i) c3[i] = a[i] * (1 - t4) + b[i] * t4;
    return c3;
  };
}
...
function genericArray2(a, b) {
  ...
  for (i = 0; i < na; ++i) x[i] = value_default2(a[i], b[i]);
  for (; i < nb; ++i) c3[i] = b[i];
  return function(t4) {
    for (i = 0; i < na; ++i) c3[i] = x[i](t4);
    return c3;
  };
}
...
function loggish(transform) {
  ...
  scale.ticks = function(count2) {
    ...
    if (!(base % 1) && j2 - i < n2) {
      ...
      if (z2.length * 2 < n2) z2 = ticks_default(u, v, n2);
    } else {
...
function newInterval(floori, offseti, count2, field) {
  ...
  interval2.range = function(start3, stop, step) {
    ...
    if (!(start3 < stop) || !(step > 0)) return range;
    do
...
function ticker(year2, month2, week, day2, hour2, minute2) {
  ...
  function tickInterval(start3, stop, count2) {
    ...
    if (i === tickIntervals.length) return year2.every(tickStep(start3 / durationYear, stop / durationYear, count2));
    if (i === 0) return millisecond_default.every(Math.max(tickStep(start3, stop, count2), 1));
...
function rgbConvert3(o2) {
  if (!(o2 instanceof Color3)) o2 = color3(o2);
  if (!o2) return new Rgb3();
  o2 = o2.rgb();
...
function hslConvert3(o2) {
  if (o2 instanceof Hsl3) return new Hsl3(o2.h, o2.s, o2.l, o2.opacity);
  if (!(o2 instanceof Color3)) o2 = color3(o2);
  if (!o2) return new Hsl3();
  if (o2 instanceof Hsl3) return o2;
  o2 = o2.rgb();
  var r2 = o2.r / 255, g = o2.g / 255, b = o2.b / 255, min3 = Math.min(r2, g, b), max3 = Math.max(r2, g, b), h2 = NaN, s = max3 - min3, l = (max3 + min3) / 2;
  if (s) {
    if (r2 === max3) h2 = (g - b) / s + (g < b) * 6;
...
function labConvert3(o2) {
  if (o2 instanceof Lab3) return new Lab3(o2.l, o2.a, o2.b, o2.opacity);
  if (o2 instanceof Hcl3) return hcl2lab3(o2);
  if (!(o2 instanceof Rgb3)) o2 = rgbConvert3(o2);
...
function hclConvert3(o2) {
  if (o2 instanceof Hcl3) return new Hcl3(o2.h, o2.c, o2.l, o2.opacity);
  if (!(o2 instanceof Lab3)) o2 = labConvert3(o2);
  if (o2.a === 0 && o2.b === 0) return new Hcl3(NaN, 0 < o2.l && o2.l < 100 ? 0 : NaN, o2.l, o2.opacity);
...
function hcl2lab3(o2) {
  if (isNaN(o2.h)) return new Lab3(o2.l, 0, 0, o2.opacity);
...
function cubehelixConvert3(o2) {
  if (o2 instanceof Cubehelix3) return new Cubehelix3(o2.h, o2.s, o2.l, o2.opacity);
  if (!(o2 instanceof Rgb3)) o2 = rgbConvert3(o2);
...
function M() {
  return (M = Object.assign || function(t4) {
    for (var n2 = 1; n2 < arguments.length; n2++) {
      var r2 = arguments[n2];
      for (var e3 in r2) Object.prototype.hasOwnProperty.call(r2, e3) && (t4[e3] = r2[e3]);
    }
    return t4;
...
function A4(t4, n2, r2, e3) {
  if (!n2.sprite) {
    ...
    for (--e3; ++e3 < l; ) {
      ...
      if (c3 > f && (f = c3), i + s >= 2048 && (i = 0, u += f, f = 0), u + c3 >= 2048) break;
    ...
    for (var v = o2.getImageData(0, 0, 2048 / a, 2048 / a).data, g = []; --e3 >= 0; ) if ((n2 = r2[e3]).hasText) {
      for (var b = (s = n2.width) >> 5, w = (c3 = n2.y1 - n2.y0, 0); w < c3 * b; w++) g[w] = 0;
      if (null == (i = n2.xoff)) return;
      u = n2.yoff;
...
function I(t4, n2, r2) {
  for (var e3, o2 = t4.sprite, a = t4.width >> 5, i = t4.x - (a << 4), u = 127 & i, f = 32 - u, l = t4.y1 - t4.y0, s = (t4.y + t4.y0) * (r2 >>= 5) + (i >> 5), c3 = 0; c3 < l; c3++) {
    e3 = 0;
    for (var h2 = 0; h2 <= a; h2++) if ((e3 << f | (h2 < a ? (e3 = o2[c3 * a + h2]) >>> u : 0)) & n2[s + h2]) return true;
    s += r2;
  }
  return false;
}
...
function U(t4) {
  ...
  (n2 = x ? function() {
    ...
    function x2(n4, r4, e6) {
      for (var o4, a3, i2, u3, l2, s2 = r4.x, c4 = r4.y, d3 = Math.sqrt(t5[0] * t5[0] + t5[1] * t5[1]), m3 = f2(t5), y2 = h2() < 0.5 ? 1 : -1, p = -y2; (o4 = m3(p += y2)) && (a3 = ~~o4[0], i2 = ~~o4[1], !(Math.min(Math.abs(a3), Math.abs(i2)) >= d3)); ) if (r4.x = s2 + a3, r4.y = c4 + i2, !(r4.x + r4.x0 < 0 || r4.y + r4.y0 < 0 || r4.x + r4.x1 > t5[0] || r4.y + r4.y1 > t5[1] || e6 && I(r4, n4, t5[0]) || e6 && !((u3 = r4).x + u3.x1 > (l2 = e6)[0].x && u3.x + u3.x0 < l2[1].x && u3.y + u3.y1 > l2[0].y && u3.y + u3.y0 < l2[1].y))) {
        for (var x3, v = r4.sprite, g = r4.width >> 5, b = t5[0] >> 5, w2 = r4.x - (g << 4), M2 = 127 & w2, z3 = 32 - M2, k2 = r4.y1 - r4.y0, W2 = (r4.y + r4.y0) * b + (w2 >> 5), S2 = 0; S2 < k2; S2++) {
          x3 = 0;
          for (var O2 = 0; O2 <= g; O2++) n4[W2 + O2] |= x3 << z3 | (O2 < g ? (x3 = v[S2 * g + O2]) >>> M2 : 0);
          W2 += b;
        }
    ...
    return void 0 === J ? 30 * (~~(6 * Y()) - 3) : function(t5, n3, r3) {
      ...
      if (1 === t5) e4 = [n3[0]];
        ...
        e4 = [].concat(n3);
        for (var o3 = (n3[1] - n3[0]) / (t5 - 1), a2 = n3[0] + o3; a2 < n3[1]; ) e4.push(a2), a2 += o3;
      }
      return P(e4, r3);
    }(J, K4, Y);
...
function G(i) {
  var f = i.callbacks, l = i.maxWords, s = void 0 === l ? 100 : l, c3 = i.minSize, h2 = i.options, d2 = i.size, m2 = i.words, y = function(t4, n2) {
    ...
    for (e3 = 0; e3 < a.length; e3++) n2.indexOf(r2 = a[e3]) >= 0 || (o2[r2] = t4[r2]);
    return o2;
